...
export default async function AlbumStatsPage({ params, searchParams }: PageProps) {
  const { slug } = await params;
  const params2 = await searchParams;
  const scope = await getAnalyticsScope(params2.scope);

  const album = await getAlbumBySlugAdmin(slug);

  if (!album) {
    redirect("/admin/albums");
  }

  const data = await getEntityAnalyticsData("album", album.id, scope);
  ...
  return (
    <>
      <div className="mb-4 relative">
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="flex-1">
            <AdminPageTitle
              title={album.title}
              description="View and analyze performance metrics for this album."
              entityType="album"
            />
          </div>
      ...
      <EntityStatsDashboard data={data} entityType="album" entityTitle={album.title} />
    </>
  );
}

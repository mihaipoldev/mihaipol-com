...
function getSupabaseClient(): SupabaseClient {
  ...
  if (!supabaseUrl || !supabaseAnonKey) {
    ...
    throw new Error(`Missing Supabase environment variables. Please check ${which} in your environment variables.`)
  }

  // Check for placeholder values
  if (supabaseUrl.includes('your-supabase-project-url') || supabaseAnonKey.includes('your-supabase-anon-key')) {
    const which = isServer
      ? 'SUPABASE_URL and SUPABASE_ANON_KEY'
      : 'NEXT_PUBLIC_SUPABASE_URL and NEXT_PUBLIC_SUPABASE_ANON_KEY'
    throw new Error(`Supabase environment variables contain placeholder values. Please update ${which} in your .env.local with actual values from your Supabase project settings.`)
  }
  ...
  try {
    ...
    throw new Error(`Invalid ${which} format: "${supabaseUrl}". It must be a valid HTTP or HTTPS URL (e.g., https://your-project.supabase.co).`)
  }

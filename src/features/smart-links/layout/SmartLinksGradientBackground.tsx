...
type SmartLinksGradientBackgroundProps = {
  coverImageUrl: string | null | undefined;
  children: React.ReactNode;
  useAbsolutePositioning?: boolean;
};

// Context to provide smart links colors to children
const SmartLinksColorsContext = createContext<{
  colors: string[];
  textColor: string;
  mutedColor: string;
  cardBgColor: string;
...
// Export hook to use smart links colors (keeping useAlbumColors name for backward compatibility)
export function useAlbumColors() {
  return useContext(SmartLinksColorsContext);
}
...
export default function SmartLinksGradientBackground({
  coverImageUrl,
  children,
  useAbsolutePositioning = false,
}: SmartLinksGradientBackgroundProps) {
  ...
  if (isLoading || gradientColors.length === 0) {
    ...
    return (
      <SmartLinksColorsContext.Provider
        value={{
          colors: [],
